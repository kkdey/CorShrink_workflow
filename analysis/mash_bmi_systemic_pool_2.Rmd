---
title: "MASH on systemic BMI study (pool 2)"
author: "Kushal K Dey"
date: "6/21/2017"
...
```{r}
mash_dat <- get(load("../output/mash_bmi_canonical_pool_2.rda"))
```
...
```{r}
write.table(imp_genes, file = "../utilities/bmi_mash_3/brain.txt", quote = FALSE, row.names = FALSE, col.names = FALSE)
```

```{r}
pathway2 <- read.delim("../utilities/bmi_mash_3/brain_pathway.tab")
...
```{r}
write.table(imp_genes, file = "../utilities/bmi_mash_3/nonbrain.txt", quote = FALSE, row.names = FALSE, col.names = FALSE)
```

```{r}
pathway2 <- read.delim("../utilities/bmi_mash_3/nonbrain_pathway.tab")
if(length(which(pathway2$q.value < 0.05)) > 2){
  pathway2 <- pathway2[which(pathway2$q.value < 0.05),]
}
maxpath = 75
...
```{r}
indices <- match("ENSG00000125648", gene_names_1)
```
...
```{r}
indices <- match("ENSG00000188933", gene_names_1)
```
...
```{r}
write.table(imp_genes, file = "../utilities/bmi_mash_3/blood.txt", quote = FALSE, row.names = FALSE, col.names = FALSE)
```

```{r}
pathway2 <- read.delim("../utilities/bmi_mash_3/blood_pathway.tab")
if(length(which(pathway2$q.value < 0.05)) > 2){
  pathway2 <- pathway2[which(pathway2$q.value < 0.05),]
}
maxpath = 75
...
```{r}
indices <- match("ENSG00000260461", gene_names_1)
```
...
```{r}
write.table(imp_genes, file = "../utilities/bmi_mash_3/adipose.txt", quote = FALSE, row.names = FALSE, col.names = FALSE)
```

```{r}
pathway2 <- read.delim("../utilities/bmi_mash_3/adipose_pathway.tab")
if(length(which(pathway2$q.value < 0.05)) > 2){
  pathway2 <- pathway2[which(pathway2$q.value < 0.05),]
}
maxpath = 75
...
```{r}
write.table(imp_genes, file = "../utilities/bmi_mash_3/skin.txt", quote = FALSE, row.names = FALSE, col.names = FALSE)
```
...
```{r}
num_lfsr_below_thresh <- apply(mash_dat$result$lfdr, 1, function(x) return(length(which(x < 0.01))))
...
```{r}
write.table(imp_genes, file = "../utilities/bmi_mash_3/all.txt", quote = FALSE, row.names = FALSE, col.names = FALSE)
```

```{r}
pathway2 <- read.delim("../utilities/bmi_mash_3/all_pathway.tab")
if(length(which(pathway2$q.value < 0.05)) > 2){
  pathway2 <- pathway2[which(pathway2$q.value < 0.05),]
}
maxpath = 75
...
```{r}
indices <- match("ENSG00000272955", gene_names_1)
```
...
```{r}
indices <- match("ENSG00000235231", gene_names_1)
```
