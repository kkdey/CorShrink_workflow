---
title: "Accident Study with ash"
author: "Kushal K Dey"
date: "6/9/2017"
output: html_document
---
In this script, we look at the accident comparison problem in the context of using lfsr for the adaptive shrinkage on the tissues for each gene. Adaptive shrinkage is performed using a normal distribution centered about 0. 
...
betahat_sebetahat <- get(load("../output/accident_cor_ash.rda"))
...
```{r}
write.table(names, file = "../utilities/accident_ash/accident_genes_ash_0_05.txt", quote = FALSE, row.names = FALSE, col.names = FALSE)
...
```{r}
pathway2 <- read.delim("../utilities/accident_ash/accident_genes_0_05_pathways.tab")
...
```{r}
write.table(names, file = "../utilities/accident_ash/accident_genes_ash_0_3.txt", quote = FALSE, row.names = FALSE, col.names = FALSE)
out <- mygene::queryMany(names,  scopes="ensembl.gene", fields=c("name", "summary"), species="human");
cbind.data.frame(out$name, out$summary, out$query)
```

```{r}
pathway2 <- read.delim("../utilities/accident_ash/accident_genes_0_3_pathways.tab")
if(length(which(pathway2$q.value < 0.05)) > 2){
  pathway2 <- pathway2[which(pathway2$q.value < 0.05),]
}
maxpath = 75
...
```{r}
betahat_sebetahat_2 <- get(load("../output/accident_cor_nonmash.rda"))
...
for(j in 1:53){
  ...
  suicide_indices <- grep("Accident", manner_of_death)
  ...
  temp1_M  <- temp11[which(temp21 == "Accident")]
...
## ENSG00000213366 or GSTM2

Check the read expression profile in GTEx here 

[ENSG00000213366](https://gtexportal.org/home/gene/ENSG00000213366)
...
```{r}
indices <- match("ENSG00000213366", colnames(betahat_mat))
```
