---
title: "Arteries : Tissue wide CorShrink"
author: "Kushal K Dey"
date: "5/26/2017"
...
artery_set <- match(c("Artery - Coronary", "Artery - Aorta", "Artery - Tibial"), U)
for(m in 1:dim(corshrink_data)[3]){
  temp <- corshrink_data[artery_set, artery_set, m]
...
```{r}
high_artery_genes <- gene_names_1[ordering_high]
low_artery_genes <- gene_names_1[ordering_low]

write.table(cbind.data.frame(high_artery_genes), quote = FALSE, file = "../utilities/artery/high_artery.txt", row.names = FALSE, col.names = FALSE)

write.table(cbind.data.frame(low_artery_genes), quote = FALSE, file = "../utilities/artery/low_artery.txt", row.names = FALSE, col.names = FALSE)
```

```{r}
tab <- read.delim("../utilities/artery/pathway_high.tab")
head(cbind.data.frame(tab$pathway, tab$q.value), 10)
```

```{r fig.height = 10}
pathway2 <- read.delim("../utilities/artery/pathway_high.tab")
...
```{r}
tab <- read.delim("../utilities/artery/GO_high.tab")
head(cbind.data.frame(tab$term_name, tab$q.value), 10)
```

```{r}
tab <- read.delim("../utilities/artery/pathway_low.tab")
head(cbind.data.frame(tab$pathway, tab$q.value), 10)
```

```{r fig.height = 10}
pathway2 <- read.delim("../utilities/artery/pathway_low.tab")
if(length(which(pathway2$q.value < 0.05)) > 2){
  pathway2 <- pathway2[which(pathway2$q.value < 0.05),]
}
maxpath = 75
...
```{r}
tab <- read.delim("../utilities/artery/GO_low.tab")
head(cbind.data.frame(tab$term_name, tab$q.value), 10)
```
