---
title: "MASH on systemic sex study (pool 2)"
author: "Kushal K Dey"
date: "6/21/2017"
...
```{r}
mash_dat <- get(load("../output/mash_sex_canonical_pool_2.rda"))
```
...
```{r}
write.table(imp_genes, file = "../utilities/sex_mash_3/breast.txt", quote = FALSE, row.names = FALSE, col.names = FALSE)
```

```{r}
pathway2 <- read.delim("../utilities/sex_mash_3/breast_pathway.tab")
...
```{r}
write.table(imp_genes, file = "../utilities/sex_mash_3/brain.txt", quote = FALSE, row.names = FALSE, col.names = FALSE)
```

```{r}
pathway2 <- read.delim("../utilities/sex_mash_3/brain_pathway.tab")
if(length(which(pathway2$q.value < 0.05)) > 2){
  pathway2 <- pathway2[which(pathway2$q.value < 0.05),]
}
maxpath = 75
...
## Non Brain specific genes 

```{r}
nonbrain_specific <- apply(mash_dat$result$lfdr, 1, function(x) {
      if(max(x[-grep("Brain", U)]) < 0.05 && min(x[grep("Brain", U)])> 0.9){
        return(1)
      }else{
        return(0)
...
```{r}
lfdr_diff <- apply(mash_dat$result$lfdr, 1, function(x) {
      y <- min(x[grep("Brain", U)]) - max(x[-grep("Brain", U)])
      return(y)
})
```
...
```{r}
length(which(nonbrain_specific==1))
```

```{r}
imp_genes <- gene_names_1[which(nonbrain_specific==1)]
```

```{r}
write.table(imp_genes, file = "../utilities/sex_mash_3/nonbrain.txt", quote = FALSE, row.names = FALSE, col.names = FALSE)
```

```{r}
pathway2 <- read.delim("../utilities/sex_mash_3/nonbrain_pathway.tab")
if(length(which(pathway2$q.value < 0.05)) > 2){
  pathway2 <- pathway2[which(pathway2$q.value < 0.05),]
}
maxpath = 75
...
```{r}
indices <- match("ENSG00000255071", gene_names_1)
```
...
```{r}
indices <- match("ENSG00000134339", gene_names_1)
```
...
```{r}
write.table(imp_genes, file = "../utilities/sex_mash_3/adipose.txt", quote = FALSE, row.names = FALSE, col.names = FALSE)
```

```{r}
pathway2 <- read.delim("../utilities/sex_mash_3/adipose_pathway.tab")
if(length(which(pathway2$q.value < 0.05)) > 2){
  pathway2 <- pathway2[which(pathway2$q.value < 0.05),]
}
maxpath = 75
...
```{r}
write.table(imp_genes, file = "../utilities/sex_mash_3/skin.txt", quote = FALSE, row.names = FALSE, col.names = FALSE)
```

```{r}
pathway2 <- read.delim("../utilities/sex_mash_3/skin_pathway.tab")
if(length(which(pathway2$q.value < 0.05)) > 2){
  pathway2 <- pathway2[which(pathway2$q.value < 0.05),]
}
maxpath = 75
...
```{r}
indices <- match("ENSG00000198018", gene_names_1)
```
...
```{r}
write.table(imp_genes, file = "../utilities/sex_mash_3/muscle.txt", quote = FALSE, row.names = FALSE, col.names = FALSE)
```

```{r}
pathway2 <- read.delim("../utilities/sex_mash_3/muscle_pathway.tab")
if(length(which(pathway2$q.value < 0.05)) > 2){
  pathway2 <- pathway2[which(pathway2$q.value < 0.05),]
}
maxpath = 75
...
```{r}
write.table(imp_genes, file = "../utilities/sex_mash_3/all.txt", quote = FALSE, row.names = FALSE, col.names = FALSE)
```

```{r}
pathway2 <- read.delim("../utilities/sex_mash_3/all_pathway.tab")
if(length(which(pathway2$q.value < 0.05)) > 2){
  pathway2 <- pathway2[which(pathway2$q.value < 0.05),]
}
maxpath = 75
