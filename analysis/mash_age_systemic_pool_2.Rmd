---
title: "MASH on systemic age study (pool 2)"
author: "Kushal K Dey"
date: "6/21/2017"
...
```{r}
mash_dat <- get(load("../output/mash_age_canonical_pool_2.rda"))
```
...
```{r}
write.table(imp_genes, file = "../utilities/age_mash_3/brain.txt", quote = FALSE, row.names = FALSE, col.names = FALSE)
```

```{r}
pathway2 <- read.delim("../utilities/age_mash_3/brain_pathway.tab")
...
```{r}
write.table(imp_genes, file = "../utilities/age_mash_3/nonbrain.txt", quote = FALSE, row.names = FALSE, col.names = FALSE)
```

```{r}
pathway2 <- read.delim("../utilities/age_mash_3/nonbrain_pathway.tab")
if(length(which(pathway2$q.value < 0.05)) > 2){
  pathway2 <- pathway2[which(pathway2$q.value < 0.05),]
}
maxpath = 75
...
```{r}
indices <- match("ENSG00000198730", gene_names_1)
```
...
```{r}
write.table(imp_genes, file = "../utilities/age_mash_3/all.txt", quote = FALSE, row.names = FALSE, col.names = FALSE)
```

```{r}
pathway2 <- read.delim("../utilities/age_mash_3/all_pathway.tab")
if(length(which(pathway2$q.value < 0.05)) > 2){
  pathway2 <- pathway2[which(pathway2$q.value < 0.05),]
}
maxpath = 75
...
```{r}
indices <- match("ENSG00000176871", gene_names_1)
```
