---
title: 'Blood and Spleen : Genewide CorShrink'
author: "Kushal K Dey"
date: "5/29/2017"
...
blood_spleen_2_set <- match(c("Whole Blood", "Spleen"), U)
for(m in 1:dim(corshrink_data)[3]){
  temp <- corshrink_data[blood_spleen_2_set, blood_spleen_2_set, m]
...
```{r}
high_blood_spleen_2_genes <- gene_names_1[ordering_high]
low_blood_spleen_2_genes <- gene_names_1[ordering_low]

write.table(cbind.data.frame(high_blood_spleen_2_genes), quote = FALSE, file = "../utilities/blood_spleen_2/high_blood_spleen_2.txt", row.names = FALSE, col.names = FALSE)

write.table(cbind.data.frame(low_blood_spleen_2_genes), quote = FALSE, file = "../utilities/blood_spleen_2/low_blood_spleen_2.txt", row.names = FALSE, col.names = FALSE)
```

```{r}
tab <- read.delim("../utilities/blood_spleen_2/pathway_high.tab")
head(cbind.data.frame(tab$pathway, tab$q.value), 10)
```

```{r fig.height = 10}
pathway2 <- read.delim("../utilities/blood_spleen_2/pathway_high.tab")
...
```{r}
tab <- read.delim("../utilities/blood_spleen_2/GO_high.tab")
head(cbind.data.frame(tab$term_name, tab$q.value), 10)
```

```{r}
tab <- read.delim("../utilities/blood_spleen_2/pathway_low.tab")
head(cbind.data.frame(tab$pathway, tab$q.value), 10)
```

```{r fig.height = 10}
pathway2 <- read.delim("../utilities/blood_spleen_2/pathway_low.tab")
if(length(which(pathway2$q.value < 0.05)) > 2){
  pathway2 <- pathway2[which(pathway2$q.value < 0.05),]
}
maxpath = 75
...
```{r}
tab <- read.delim("../utilities/blood_spleen_2/GO_low.tab")
head(cbind.data.frame(tab$term_name, tab$q.value), 10)
```
