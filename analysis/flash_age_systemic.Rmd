---
title: "FLASH on systemic age study"
author: "Kushal K Dey"
date: "6/14/2017"
...
```{r}
indices <- order(abs(flash_out$f[,9]), decreasing=TRUE)[1:50]
par(mfrow=c(3,3))
for(m in 1:50){
  plot(1:53, zdat[order_index,indices[m]], pch=20, type="b", xlab = "tissues", ylab="z scores")
  text(1:53, zdat[order_index,indices[m]], U[order_index], cex=0.3)
...
```{r}
indices <- order(abs(flash_out$f[,10]), decreasing=TRUE)[1:50]
par(mfrow=c(3,3))
for(m in 1:50){
  plot(1:53, zdat[order_index,indices[m]], pch=20, type="b", xlab = "tissues", ylab="z scores")
  text(1:53, zdat[order_index,indices[m]], U[order_index], cex=0.3)
...
```{r}
pathway2 <- read.delim("../utilities/age_flash/brain_pathway.tab")
...
```{r}
write.table(imp_genes_names[1:100], file = "../utilities/age_flash/blood.txt", quote = FALSE, row.names = FALSE, col.names = FALSE)
out <- mygene::queryMany(imp_genes_names[1:50],  scopes="ensembl.gene", fields=c("name", "summary"), species="human");
cbind.data.frame(out$name, out$summary)
```

```{r}
pathway2 <- read.delim("../utilities/age_flash/blood_pathway.tab")
if(length(which(pathway2$q.value < 0.05)) > 2){
  pathway2 <- pathway2[which(pathway2$q.value < 0.1),]
}
maxpath = 75
...
```{r}
write.table(imp_genes_names, file = "../utilities/age_flash/adipose.txt", quote = FALSE, row.names = FALSE, col.names = FALSE)
out <- mygene::queryMany(imp_genes_names[1:50],  scopes="ensembl.gene", fields=c("name", "summary"), species="human");
cbind.data.frame(out$name, out$summary)
```

```{r}
pathway2 <- read.delim("../utilities/age_flash/adipose_pathway.tab")
if(length(which(pathway2$q.value < 0.05)) > 2){
  pathway2 <- pathway2[which(pathway2$q.value < 0.1),]
}
maxpath = 75
...
```{r}
write.table(imp_genes_names, file = "../utilities/age_flash/artery.txt", quote = FALSE, row.names = FALSE, col.names = FALSE)
out <- mygene::queryMany(imp_genes_names[1:50],  scopes="ensembl.gene", fields=c("name", "summary"), species="human");
cbind.data.frame(out$name, out$summary)
```

```{r}
pathway2 <- read.delim("../utilities/age_flash/artery_pathway.tab")
if(length(which(pathway2$q.value < 0.05)) > 2){
  pathway2 <- pathway2[which(pathway2$q.value < 0.1),]
}
maxpath = 75
...
### ENSG00000176476 (Brain specific)

```{r}
indices <- match("ENSG00000176476", gene_names_1)
```
...
## ENSG00000133315 (Brain specific)

```{r}
indices <- match("ENSG00000133315", gene_names_1)
```
...
### ENSG00000273002 (Blood specific)

```{r}
indices <- match("ENSG00000273002", gene_names_1)
```
...
### ENSG00000035664 (Artery specific)

```{r}
indices <- match("ENSG00000035664", gene_names_1)
```
