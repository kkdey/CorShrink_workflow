...
betahat_sebetahat <- get(load("../output/cva_v_anoxia_cor_ash.rda"))
...
imp_genes <- order(num_lfsr_below_thresh, decreasing = TRUE)[1:50]
...
```{r}
write.table(names, file = "../utilities/cva_anoxia/cva_anoxia_genes_ash_0_05.txt", quote = FALSE, row.names = FALSE, col.names = FALSE)
...
```{r}
pathway2 <- read.delim("../utilities/cva_anoxia/cva_anoxia_genes_0_05_pathways.tab")
...
```{r}
sample_attributes$DTHTIME[cva_indices]
```
...
```{r}
plot(density(times_1[cva_indices], na.rm = TRUE), main = "Anoxia death times distribution")
plot(density(times_1[anoxia_indices], na.rm=TRUE), main = "CVA death times distribution")
```

```{r}
sample_attributes$DTHMNNR[cva_indices]
```

We now look at the top 2 genes we found from above.

## ENSG00000204520 or MICA

Check the read expression profile in GTEx here 

[ENSG00000204520](https://gtexportal.org/home/gene/ENSG00000204520)

```{r}
betahat_sebetahat_2 <- get(load("../output/cva_v_anoxia_cor_nonmash.rda"))
tmat <- betahat_sebetahat_2$betahat/betahat_sebetahat_2$sebetahat
```

```{r}
indices <- match("ENSG00000204520", colnames(betahat_mat))
```
...
## ENSG00000152926 or ZNF117

Check the read expression profile in GTEx here 

[ENSG00000152926](https://gtexportal.org/home/gene/ENSG00000152926)
...
```{r}
indices <- match("ENSG00000152926", colnames(betahat_mat))
```
