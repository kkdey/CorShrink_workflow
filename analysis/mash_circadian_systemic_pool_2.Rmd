---
title: "MASH on systemic circadian studies (pool 2)"
author: "Kushal K Dey"
date: "6/18/2017"
...
```{r}
mash_dat <- get(load("../output/mash_circadian_canonical_pool_2.rda"))
```
...
```{r}
write.table(imp_genes, file = "../utilities/circadian_mash_3/liver.txt", quote = FALSE, row.names = FALSE, col.names = FALSE)
```

```{r}
pathway2 <- read.delim("../utilities/circadian_mash_3/liver_pathway.tab")
...
```{r}
write.table(imp_genes, file = "../utilities/circadian_mash_3/brain.txt", quote = FALSE, row.names = FALSE, col.names = FALSE)
```

```{r}
pathway2 <- read.delim("../utilities/circadian_mash_3/brain_pathway.tab")
if(length(which(pathway2$q.value < 0.05)) > 2){
  pathway2 <- pathway2[which(pathway2$q.value < 0.05),]
}
maxpath = 75
...
nonbrain_specific <- apply(mash_dat$result$lfdr, 1, function(x) {
      if(max(x[-grep("Brain", U)]) < 0.05 && min(x[grep("Brain", U)])> 0.5){
        return(1)
      }else{
        return(0)
      }
...
```{r}
write.table(imp_genes, file = "../utilities/circadian_mash_3/nonbrain.txt", quote = FALSE, row.names = FALSE, col.names = FALSE)
```

```{r}
pathway2 <- read.delim("../utilities/circadian_mash_3/nonbrain_pathway.tab")
if(length(which(pathway2$q.value < 0.05)) > 2){
  pathway2 <- pathway2[which(pathway2$q.value < 0.05),]
}
maxpath = 75
...
```{r}
indices <- match("ENSG00000164308", gene_names_1)
```
...
```{r}
indices <- match("ENSG00000126368", gene_names_1)
```
...
```{r}
write.table(imp_genes, file = "../utilities/circadian_mash_3/all.txt", quote = FALSE, row.names = FALSE, col.names = FALSE)
```

```{r}
pathway2 <- read.delim("../utilities/circadian_mash_3/all_pathway.tab")
if(length(which(pathway2$q.value < 0.05)) > 2){
  pathway2 <- pathway2[which(pathway2$q.value < 0.05),]
}
maxpath = 75
