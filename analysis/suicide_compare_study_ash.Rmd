---
title: "Suicide study with ash"
author: "Kushal K Dey"
date: "6/8/2017"
output: html_document
---

In this script, we look at the Suicide comparison problem in the context of using lfsr for the adaptive shrinkage on the tissues for each gene. Adaptive shrinkage is performed using a normal distribution centered about 0. 
...
```
```{r}
write.table(names, file = "../utilities/suicide_ash/suicide_genes_ash_0_1.txt", quote = FALSE, row.names = FALSE, col.names = FALSE)
...
```
We do not get any pathway with this. So, we relax the lfsr threshold a bit.
...
```{r}
write.table(names, file = "../utilities/suicide_ash/suicide_genes_ash_0_3.txt", quote = FALSE, row.names = FALSE, col.names = FALSE)
out <- mygene::queryMany(names,  scopes="ensembl.gene", fields=c("name", "summary"), species="human");
cbind.data.frame(out$name, out$summary, out$query)
```

```{r}
pathway2 <- read.delim("../utilities/suicide_ash/suicide_genes_0_3_pathways.tab")
...
Now there seem to be some Circadian genes that show up in this analysis. Are these genes meaningful?

```{r}
manner_of_death <- sample_attributes$DTHMNNR
suicide_indices <- grep("Suicide", manner_of_death)
time_of_suicides <- sample_attributes$DTHTIME[suicide_indices]
time_of_suicides
```

We plot the density plot of these suicide times.
...
```{r}
plot(density(times_1[-suicide_indices], na.rm = TRUE), main = "Non suicide death times distribution")
plot(density(times_1[suicide_indices], na.rm=TRUE), main = "Suicide death times distribution")
```

The suicide death times seem to be more concentrated towards the noon compared to the non suicidal deaths.

Is that the reason the Circadian genes are found to be important?

This [Article](https://www.ncbi.nlm.nih.gov/pmc/articles/PMC4285016/) does mention that circadian genes do play a role in mood disorders. Mood regulation is known to be affected by the change of seasons. Recent research findings have suggested that mood regulation may be influenced by the function of circadian clocks. In addition, the activity of brown adipocytes has been hypothesized to contribute to mood regulation. 

Dysfunction of the proteins encoded from the circadian clock genes is hypothesized to play a role in the etiology of mood disorders (9). Here, I consider those proteins that are repressors of transcription to be most important, since they are essential to the normal function of circadian clocks (10). Among them, nuclear receptor subfamily 1, group D, member 1 (NR1D1) has a key position as a connecting node in the transcriptional and translational loops that constitute the circadian clock in a cell. 

CRY2 and CRY1 are also related to depressive disorders. They are the key repressors of the Circadian clock. CRY2 opposes the actions of PER1.  

so, what are the Circadian genes found by this approach? They are **ENSG00000126368** or **NR1D1** and **ENSG00000179094** or **PER1**.

## ENSG00000126368 or NR1D1
...
```{r}
betahat_sebetahat_2 <- get(load("../output/suicide_cor_nonmash.rda"))
tmat <- betahat_sebetahat_2$betahat/betahat_sebetahat_2$sebetahat
```

```{r}
indices <- match("ENSG00000126368", colnames(betahat_mat))
```
...
```{r}
indices <- match("ENSG00000179094", colnames(betahat_mat))
```
...
## ENSG00000107815 or TWNK

Check the read expression profile in GTEx here 

[ENSG00000107815](https://gtexportal.org/home/gene/ENSG00000107815)
...
```{r}
indices <- match("ENSG00000107815", colnames(betahat_mat))
```
...
## DAPK2 or ENSG00000035664

Check the read expression profile in GTEx here 

[ENSG00000035664](https://gtexportal.org/home/gene/ENSG00000035664)
...
```{r}
indices <- match("ENSG00000035664", colnames(betahat_mat))
```
