...
covshrink_ML <- CorShrink::CovShrink(voom_deng_counts, 20, type="ML");
corshrink_VEM <- CorShrink::CovShrink(voom_deng_counts, 10, type="VEM");
save(corshrink_ML, file="../corshrink_ML_genes.rda")
save(corshrink_VEM, file="../corshrink_VEM_genes.rda")


#corshrink_VEM2 <- CorShrink::CovShrink(t(voom_deng_counts, 6, type="VEM2");
corshrink_strimmer <- corpcor::cov.shrink(cor_sample);

save(corshrink_ML, file="../corshrink_ML.rda")
save(corshrink_VEM, file="../corshrink_VEM.rda")
save(corshrink_VEM2, file="../corshrink_VEM2.rda")
save(corshrink_strimmer, file="../corshrink_strimmer.rda")

par(mfrow=c(1,1))
cols <- rainbow(100)
#fields::image.plot(cov2cor(corshrink_strimmer), breaks = 0:100/100, col=cols, nlevel=100, main = "Shafer-Strimmer", cex.main = 0.5)
fields::image.plot(cor_sample, breaks = 0:100/100, col=cols, nlevel=100, main="Sample corr. ")
fields::image.plot(cov2cor(as.matrix(corshrink_ML)), breaks = 0:100/100, col=cols, nlevel=100, main = "CorShrink-ML", cex.main = 0.5)
fields::image.plot(cov2cor(as.matrix(corshrink_VEM)), breaks = 0:100/100, col=cols, nlevel=100, main = "CorShrink-VEM", cex.main = 0.5)
...
covshrink_ML <- get(load("../corshrink_ML.rda"))
...
fac <- factor(deng.meta_data$cell_type, levels =  c("zy", "early2cell", "mid2cell", "late2cell", "4cell", "8cell",
                                                    "16cell", "earlyblast", "midblast", "lateblast"))

indices_vertex <- unique(fac)

pr <- prcomp(dat);
plot(pr$x[,1], pr$x[,2], col=color[fac],
     pch=20, cex=1)
legend("topright", legend=indices_vertex,  fill=color[indices_vertex],
       cex=0.5)

plot(pr_projected$x[,1], pr_projected$x[,2], col=color[fac],
     pch=20, cex=1)
legend("topright", legend=indices_vertex,  fill=color[indices_vertex],
       cex=0.5)
...
fields::image.plot(cov2cor(as.matrix(corshrink_VEM2)), breaks = 0:100/100, col=cols, nlevel=100)
...
library(expm)
projected_dat <- sqrtm(corshrink_ML)%*% sqrtm(solve(cov_sample)) %*% dat
pr_projected <- prcomp(projected_dat)
...
color=c("blue","black","cyan",
        "brown4","darkgoldenrod1","darkkhaki",
        "darkorchid","green","magenta", "azure1","azure4");
...
legend("topright", legend=indices_vertex,  fill=color[factor(indices_vertex)],
       cex=0.5)
